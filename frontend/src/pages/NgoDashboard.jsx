...
Return ONLY valid JSON, no markdown, no extra text:
{
  ...
  try {
    ...
    console.error("NGO Verification API Error:", err);
    return {
      document_classification:'api_error', confidence_score:50, decision:'pending_retry',
      matched_fields:{ organization_name:null, registration_number:null, location:null, purpose:null },
      extracted_text_summary:'AI service temporarily unavailable. Verification delayed.',
      red_flags:[],
      reasoning:'AI verification delayed due to provider outage. Saved to queue for retry.',
      extractedOrgName:null, extractedRegNumber:null, extractedAuthority:null, extractedDate:null,
      nameMatch:null, regNumberMatch:null, documentAuthenticityScore:50,
      documentType:'Pending Verification', redFlags:[], summary:'Verification delayed.',
...
function Field({ label, required, error, touched, hint, children }) {
  ...
  return (
    <div data-field-error={hasError ? "true" : undefined}>
      ...
      {children}
      {hasError && <div style={ERR}>⚠ {error}</div>}
    </div>
  );
}
...
export default function NgoDashboard() {
  ...
  const handleSubmit = async () => {
    ...
    if (missingDocs.length > 0) {
      setDocErrors(e => {
        const next = { ...e };
        missingDocs.forEach(d => next[d.key] = 'Required document is missing');
        return next;
      });
      hasDocErrors = true;
    }
    ...
    if (fieldErrors.length > 0 || hasDocErrors) {
      setTimeout(() => {
