...
const REQUIRED_DOCS = [
  { key: 'regCertificate', label: 'Registration / Incorporation Certificate', required: true,
    hint: 'Society registration, Trust deed, or Section 8 company certificate' },
  { key: 'panCard',        label: 'PAN Card of Organisation',                 required: true,
    hint: "PAN card issued in the organisation's name (not personal)" },
  { key: 'authLetter',     label: 'Authorisation Letter',                     required: true,
    hint: 'Letter from chairman/board authorising this person to represent the org' },
  { key: 'cert80G',        label: '80G / 12A Tax Exemption Certificate',      required: false,
    hint: 'Required for donors to claim income-tax deduction on their donations' },
  { key: 'auditReport',    label: 'Latest Audited Financial Report',           required: false,
    hint: 'Annual audit report for the most recent financial year' },
];
...
const VALIDATORS = {
  ...
  yearEstablished: v => {
    ...
    if (isNaN(yr) || yr < 1800 || yr > now) return `Enter a year between 1800 and ${now}`;
    return null;
  },
  ...
  website:      v => {
    ...
    try { new URL(v.trim()); return null; } catch { return 'Enter a valid URL (e.g. https://example.org)'; }
  },
...
const input = (err, touched) => ({
  ...INPUT_BASE,
  border: err && touched ? '1px solid rgba(239,68,68,0.7)' : '1px solid rgba(255,255,255,0.12)',
});
const LABEL = {
...
function uploadToCloudinary(file, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    formData.append('folder', 'ngoRequests');
  ...
  return (
    <div>
      ...
      <div onClick={() => ref.current.click()} style={{
        ...INPUT_BASE, cursor: 'pointer', border: `1px solid ${borderColor}`,
        ...
        <input ref={ref} type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display: 'none' }}
          onChange={e => onChange(docDef.key, e.target.files[0])} />
      ...
      {value && !error && (
        <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)', marginTop: '4px' }}>
          {(value.size / 1024 / 1024).toFixed(2)} MB{value.type.startsWith('image/') && ' · will be compressed'}
        </div>
      )}
    </div>
  );
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div style={{ width: '100%', maxWidth: '460px', borderRadius: '24px', border: '1px solid rgba(16,185,129,0.4)', background: '#0d1021', padding: '40px', textAlign: 'center', boxShadow: '0 0 60px rgba(16,185,129,0.15)' }}>
        ...
        <div style={{ padding: '14px 16px', borderRadius: '12px', marginBottom: '28px', border: '1px solid rgba(16,185,129,0.3)', background: 'rgba(16,185,129,0.08)', fontSize: '13px', color: '#6ee7b7', lineHeight: 1.8, textAlign: 'left' }}>
          ...
          ✅ Upload milestone proof documents<br />
          ✅ Track fund releases in real time<br />
          ✅ Receive verified donor contributions
        </div>
...
export default function NgoDashboard() {
  ...
  const handleSubmit = async () => {
    ...
    if (Object.values(docErrors).some(Boolean)) {
    ...
    try {
      ...
      // Upload one by one so progress bar is clear and accurate
    ...
    <>
      ...
      <div style={{ padding: '40px 48px', maxWidth: '900px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px', marginBottom: '28px' }}>
          {[{ label: 'Active campaigns', val: '0', color: '#a78bfa' }, { label: 'Total raised', val: '₹0', color: '#22d3ee' }, { label: 'Funds released', val: '₹0', color: '#34d399' }].map(s => (
    ...
    <div style={{ minHeight: 'calc(100vh - 68px)', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '48px 16px 80px' }}>
      <div style={{ width: '100%', maxWidth: '720px' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '14px', marginBottom: '32px', lineHeight: 1.6 }}>
          Fill in your details and upload the required documents. An admin will verify and approve your application.
        </p>
        ...
        <div style={{ borderRadius: '20px', border: '1px solid rgba(255,255,255,0.08)', background: '#0d1021', padding: '32px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            <Field label="Organisation name" required error={getError('orgName')} touched={touched.orgName}>
              <input value={form.orgName} onChange={e => setField('orgName', e.target.value)}
                ...
                placeholder="e.g. Helping Hands Foundation" style={input(getError('orgName'), touched.orgName)} />
            </Field>
            <Field label="Organisation type" required error={getError('orgType')} touched={touched.orgType}>
              <select value={form.orgType} onChange={e => setField('orgType', e.target.value)}
                onBlur={() => setTouched(t => ({ ...t, orgType: true }))}
                style={{ ...input(getError('orgType'), touched.orgType), WebkitAppearance: 'none', appearance: 'none', cursor: 'pointer' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            <Field label="Registration / Certificate number" required error={getError('regNumber')} touched={touched.regNumber}>
              <input value={form.regNumber} onChange={e => setField('regNumber', e.target.value)}
                onBlur={() => setTouched(t => ({ ...t, regNumber: true }))}
                placeholder="e.g. MH/NGO/12345/2018" style={input(getError('regNumber'), touched.regNumber)} />
            </Field>
            <Field label="Year established" error={getError('yearEstablished')} touched={touched.yearEstablished}>
              <input type="number" min="1800" max={new Date().getFullYear()}
                value={form.yearEstablished} onChange={e => setField('yearEstablished', e.target.value)}
                onBlur={() => setTouched(t => ({ ...t, yearEstablished: true }))}
                placeholder={`e.g. ${new Date().getFullYear() - 5}`}
                style={input(getError('yearEstablished'), touched.yearEstablished)} />
            </Field>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            <Field label="City" required error={getError('city')} touched={touched.city}>
              <input value={form.city} onChange={e => setField('city', e.target.value)}
                ...
                placeholder="e.g. Pune" style={input(getError('city'), touched.city)} />
            </Field>
            <Field label="State" required error={getError('state')} touched={touched.state}>
              <input value={form.state} onChange={e => setField('state', e.target.value)}
                onBlur={() => setTouched(t => ({ ...t, state: true }))}
                placeholder="e.g. Maharashtra" style={input(getError('state'), touched.state)} />
            </Field>
          </div>
          ...
          <div style={{ marginBottom: '16px' }}>
            <Field label="Website" error={getError('website')} touched={touched.website}>
              <input type="text" value={form.website} onChange={e => setField('website', e.target.value)}
                ...
                placeholder="https://yourorg.org" style={input(getError('website'), touched.website)} />
            </Field>
          </div>

          <div style={{ marginBottom: '28px' }}>
            <Field label="Mission & description" required error={getError('description')} touched={touched.description}
              hint={`${form.description.trim().length}/30 characters minimum`}>
              <textarea rows={4} value={form.description} onChange={e => setField('description', e.target.value)}
                onBlur={() => setTouched(t => ({ ...t, description: true }))}
                placeholder="Describe your organisation's mission, the work you do, and why you need to raise funds…"
                style={{ ...input(getError('description'), touched.description), resize: 'vertical', lineHeight: 1.65 }} />
            </Field>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '28px' }}>
            <Field label="Contact person name" required error={getError('contactName')} touched={touched.contactName}>
              <input value={form.contactName} onChange={e => setField('contactName', e.target.value)}
                ...
                placeholder="e.g. Priya Sharma" style={input(getError('contactName'), touched.contactName)} />
            </Field>
            <Field label="Contact phone" required error={getError('contactPhone')} touched={touched.contactPhone}
              hint="10-digit Indian mobile number">
              <input type="tel" maxLength={10} value={form.contactPhone}
                onChange={e => setField('contactPhone', e.target.value.replace(/\D/g, '').slice(0, 10))}
                onBlur={() => setTouched(t => ({ ...t, contactPhone: true }))}
                placeholder="e.g. 9876543210" style={input(getError('contactPhone'), touched.contactPhone)} />
            </Field>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '28px' }}>
            {REQUIRED_DOCS.map(d => (
              <FileField key={d.key} docDef={d} value={docFiles[d.key]} error={docErrors[d.key]} onChange={handleDocChange} />
            ))}
          </div>
            ...
            <div style={{ marginBottom: '20px', padding: '16px 18px', borderRadius: '14px', border: '1px solid rgba(124,58,237,0.3)', background: 'rgba(124,58,237,0.08)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                ...
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', flexShrink: 0, marginLeft: '8px' }}>
                  {doneFiles < totalFiles ? `File ${doneFiles + 1} of ${totalFiles}` : 'Saving…'}
                </div>
              </div>
              <div style={{ height: '6px', borderRadius: '6px', background: 'rgba(255,255,255,0.08)', overflow: 'hidden' }}>
                <div style={{
                  ...
                  width: doneFiles >= totalFiles ? '100%'
                    : `${Math.round((doneFiles / totalFiles) * 100 + (uploadPct / totalFiles))}%`,
              ...
              <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginTop: '6px' }}>
                {doneFiles < totalFiles
                  ...
                  : 'All files uploaded · saving to database…'}
              </div>
            </div>
          )}
