...
const CLASS_CAPS = {
  ...
  illustration: 0, artwork: 0, code_image: 0,
  screenshot: 5, wrong_document: 15, blank: 0, no_image: 0,
};
...
async function verifyDocument(campaignContext, base64Image, mimeType) {
  ...
  if (!pf.pass) {
    ...
    return buildFinalResult({
      document_classification: pf.cls,
      is_relevant:      false,
  ...
  console.log(`[VERIFIER] Prompt: ${fullPrompt.length} chars | Image: ${
    base64Image ? Math.round((base64Image.length * 3 / 4) / 1024) + 'KB' : 'none'
  }`);

  // ── Step 1: Gemini (multi-key, multi-model) ──────────────────────────────
