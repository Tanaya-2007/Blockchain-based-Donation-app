...
// Models available on FREE tier in 2026 — ordered newest to oldest
const MODEL_CASCADE = [
  'gemini-2.0-flash',
...
function enforceAllCaps(parsed) {
  ...
  if (HARD_CAPS[cls] !== undefined) {
    ...
    if ((parsed.confidence_score || 0) > cap) {
      console.warn(`[GEMINI CAPS] "${cls}" capped: ${parsed.confidence_score} to ${cap}`);
...
- AI-generated images (Gemini/DALL-E/Midjourney) MUST score 0. No exceptions.
- Unrelated photos, illustrations, artwork MUST score 0-5.
- Screenshots MUST score 0-5.
- Only real photographed/scanned paper documents can score above 40.
- Threshold for admin review is 75. Below 75 = rejected.
- Detect AI images by: perfect smooth textures, no paper grain, rendered/embedded text, too-perfect lighting, surreal quality.
...
Return ONLY valid JSON:
{
  ...
  if (parsed) {
    ...
    console.log(`[GEMINI] SUCCESS Key ${keyIdx} + ${modelName} | class: ${capped.document_classification} | score: ${capped.confidence_score}`);
    return JSON.stringify(capped);
  }
  console.log(`[GEMINI] SUCCESS Key ${keyIdx} + ${modelName} (raw)`);
  return text;
}
  ...
  for (let ki = 0; ki < keys.length; ki++) {
    for (const modelName of MODEL_CASCADE) {
      try {
        ...
        if (isModelGone)             continue; // try next model in cascade
        if (isKeyDead || isQuota)    break;    // skip to next key entirely
      }
    }
  }
